...
## Output Data
The pipeline produces several types of output data, including quality control reports, alignment files, and variant calling results. These files are stored in the output directory, which defaults to `results/` and can be redirected with `--outdir <path>` (every published output and execution report is written under it, so each run can be isolated). See the [Test the Pipeline](#test-the-pipeline) for an example of the output files generated by this pipeline.

### Execution Reports
The pipeline automatically generates Nextflow execution reports under the output directory (paths shown for the default `--outdir results`):
...
--cpus <int> Upper cap on the number of CPUs any single process may use, default: 2. Per-process resource requests now come from nf-core-style resource labels (see [Resource Management](#resource-management)); `--cpus` caps them for back-compat, so `--cpus 1` forces every task to a single core. Only the cpus available to your docker daemon can be used.
...
--max_time <str> Maximum wall time any single process may request, default: `24.h`.
...
Process resources (CPUs, memory, wall time) are managed automatically, so you no longer need to manually raise Docker's memory allocation before large runs. Each process is tagged with an nf-core-style resource **label** (`process_low`, `process_medium`, `process_high_memory`) that maps onto sensible defaults in `conf/base.config`. The memory-hungry `run_bowtie2` step (index build + alignment scales with genome size) uses the `process_high_memory` tier.

- **Auto-scaling on retry:** if a process is killed by an out-of-memory or transient error, it is retried (up to 2 times) with more memory each attempt (`memory * task.attempt`).
- **Laptop-sane caps:** every request is capped by `--max_cpus` (default 8), `--max_memory` (default `16.GB`), and `--max_time` (default `24.h`). Raise these to give the pipeline more headroom on a bigger machine, e.g. for the human genome:

```bash
nextflow run main.nf -profile docker --input_file <path> --email <ncbi-email> --max_memory '32.GB' --max_cpus 12
```

- **`--cpus` back-compat:** `--cpus` still works and acts as an upper cap on per-process CPUs (e.g. `--cpus 1` forces every task to a single core).

Make sure the CPU/memory you request is actually available to your container engine (for Docker Desktop, see `Settings > Resources`).
